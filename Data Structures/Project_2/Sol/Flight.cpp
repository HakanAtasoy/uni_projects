...
bool Flight::addTicket(const Ticket &ticket) {
    ...
    TicketType a = ticket.getTicketType() ;
    int s = tickets.size(), cap = tickets.capacity(), current = 0 ;
    for(int i = 0; i < s; i++)
    {
        if(tickets[i].getTicketType() == a) current++ ;
    }
    if(current < cap) 
    {
...
bool Flight::operator<(const Flight &rhs) const {
    ...
    const std::string code1 = this -> flightCode , code2 = rhs.getFlightCode() ;
    int a, b ;
    
    for(int i = 0, a=code1[0], b = code2[0]; a; i++)
    {
        if(!b) return false ;
        else 
        {
